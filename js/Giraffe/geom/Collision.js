...
(function(){
    var Collision = {
        ...
        checkDisplayObjectCollision: function(m1, m2){
            ...
            var isCollide = Collision.checkCollision(pos1.getX(), pos1.getY(), m1.getWidth(), m1.getHeight(),
                    pos2.getX(), pos2.getY(), m2.getWidth(), m2.getHeight());
            return isCollide;
        },
        checkGameObjectCollision: function(g1, g2){
            ...
            if(isCollide){
                ...
                if(sleep1){
                    var toSleep1 = new g.geom.Vector(pos1.getX() - pos2.getX() , pos1.getY() - pos2.getY());
                    if(toSleep1.x * v2.x <= 0 && toSleep1.y * v2.y <= 0){
                        return false;
                    }
                }
                if(sleep2){
                    var toSleep2 = new g.geom.Vector(pos2.getX() - pos1.getX() , pos2.getY() - pos1.getY());
                    if(toSleep2.x * v1.x <= 0 && toSleep2.y * v1.y <= 0){
                        return false;
                    }
                }
            }*/
        ...
        _getCenter: function(displayObject){
            var position = displayObject.getNextPosition();
            var x = position.getX();
            var y = position.getY();
